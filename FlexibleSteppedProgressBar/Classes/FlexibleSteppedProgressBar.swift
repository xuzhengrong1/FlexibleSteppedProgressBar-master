...
//        self.layer.addSublayer(self.selectionLayer)
    ...
    override open func draw(_ rect: CGRect) {
        ...
        if(!animationRendering) {
            ...
            let progressPath = self._shapePath(self.centerPoints, aRadius: _radius, aLineHeight: _lineHeight)
            ...
            if !useLastState {
                ...
                selectionCenterLayer.sublayers  = nil
                selectionCenterLayer.path = selectedPathCenter.cgPath
//                selectionCenterLayer.strokeColor = UIColor.green.cgColor //selectedOuterCircleStrokeColor.cgColor
                
                
                let image  = UIImage.init(named: "choose")
                let imageSubLayer = CALayer()
                imageSubLayer.contents = image?.cgImage
                imageSubLayer.frame = CGRect(x: 0, y: 0, width: 23, height: 23);
                imageSubLayer.position  = self.centerPoints[currentIndex]
                selectionCenterLayer.addSublayer(imageSubLayer)
    
                selectionCenterLayer.fillColor = UIColor.white.cgColor
